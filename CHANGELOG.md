...
Brings refunds and payment demands in line with the current Edge API.

### Added

- Lifecycle documentation for payment demands, refund demands and payment
  subscriptions: `docs/payment-demands.md` ("Lifecycle"), the new
  `docs/refund-demands.md` and `docs/payment-subscriptions.md`. The main point:
  `pending` means Edge has accepted a payment or refund and queued it. It does
  not mean it has reached the payment processor or the card networks, and
  `processing` is set before either is contacted too.
- README: a documentation index, an author and contact section, and a clearer
  statement that this is not an official Edge release.

### Changed

- **Partial refunds are supported.** Edge now allows several refunds against
  one payment demand, capped at its amount less every pending, processing and
  succeeded refund, enforced atomically by the server. `Edge::RefundDemand`
  documents this as a supported workflow; omitting `amount_cents` refunds the
  remaining balance, not the full amount. The client computes no balance and
  refuses no amount — the server's cap is the check, and an over-refund comes
  back as `Edge::InvalidRequestError` on `amount_cents`.
- `Edge::RefundDemand.create` sends `amount_currency: "USD"` through instead of
  refusing it; the server no longer 500s on the documented value. Any other
  value is still refused: a different real currency is a 422, but an unknown
  code alongside `amount_cents` is a 500.
- `Edge::RefundDemand` documents the refund replay contract in full: a matching
  request with the same key returns the original refund with no second refund,
  event or webhook, and a key reused for a different payment demand, amount,
  reason or note is a 422 on `idempotency_key`.
- Updated `contract/manifest.yml`. The only differences are the two retired
  states below.
- The payment demand idempotency key still does not prevent a double charge,
  so `retriable: true` is still refused on `Edge::PaymentDemand.create`.

### Removed

- `Edge::PaymentDemand#refunded?` and `refunded` in `DEMAND_STATES`. Edge
  retired the state: a refund, partial or full, leaves the demand `succeeded`,
  and existing `refunded` records were moved to `succeeded`.
- `Edge::RefundDemand#errored?` and `errored` in `STATES`. Edge removed the
  state, which no refund was ever observed in; `#settled?`
  is now `succeeded? || failed?`.
...
what the server actually does and says so in its documentation.

### Added

- Vendored API contract: `contract/manifest.yml`, describing each resource's
  operations, relationships, writeability, enums and sensitive fields.
- `docs/pagination.md`, recording the API's unpaginated behaviour.
...
- `Edge::PaymentMethod#expiry_known?`, because the API omits card expiry
  entirely today and a caller needs to tell
...
- `Edge::PaymentDemand#intent?` and `#demand?`. `POST /v2/payment_demands`
  ...
  returns it looking exactly like a payment demand — same type, same route. Nothing
...
- `Edge::Webhook`, verifying a v3 delivery: HMAC-SHA256 over
  ...
  delivery dated far ahead. Cross-checked byte for byte against real
  deliveries, including multi-byte UTF-8 in the secret and the body.
...
- `Edge::Event#code`, joining the event code the API serves as two attributes.
  What documentation and dashboards call the event —
  `transaction.payment_demands.succeeded` — is not in `slug`, which carries
  only `"succeeded"`; `resource_type` carries the rest. The joined form is never
  served or delivered, so `subscription.subscribed_to?(event.slug)` is false
  every time.
...
- `Edge::WebhookSubscription.update` refuses `status`, `secret_key`,
  `archived_at` and the merchant. The server drops all of them, so a merchant
  pausing deliveries got a `200`, a subscription
  still reporting `active?`, and deliveries still flowing.
- `Edge::RefundDemand.create` requires the payment demand it refunds and a
  reason, and refuses attributes the server ignores — `state` above all, which
  is always `pending` on create. Omitting the payment demand linkage was a 500
  rather than a validation error.
- `refund_demands.amount_currency` is refused. It is inherited from the payment
  demand, and the documented `"USD"` is answered with a 500 rather than a
  validation error.
...
- `capture_method: "manual"` is refused on payment demands. The API accepts it
  and it reaches the card network as an authorization that **nothing can
  capture or void** — it sits against the cardholder's account until it
  expires. The one place this client declines something the server allows;
  `client.post` remains the escape hatch.
- Attributes marked `readonly: true` are no longer recorded as unwritable
  wholesale. The flag is documentation only: all seven 3DS fields on
  `payment_demands` are accepted on create, and `payer_timezone` is *required*.
...
- `PaymentDemand.update` refuses the attributes `PATCH` accepts and then
  ...
  update accepts a much smaller set than create does — the 3DS
...
- `spec/contract/live_spec.rb` treats a 5xx as "not readable" and asserts the
  ...
  drift checks after it. Eight resources currently fail it.

- `payment_demands` is no longer marked `idempotent_writes`, so `retriable:`
  refuses it. Edge documents `idempotency_key` as "a unique value that
  prevents double charging"; exercised against a running instance, two
  identical requests sharing one key produced two payment demands. Refunds are
