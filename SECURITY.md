...
- **Legacy v1/v2 webhook signing offers no integrity.** That header is a
  ...
  verification.
- **A refunded total is not reported.** The Edge API caps refunds at the
  payment demand's amount, counting in-flight refunds, under a row lock. It
  does not serialize the refunded balance, so a caller that needs one sums the
  demand's `succeeded` refunds — a figure that is only as current as the list
  it came from. Summing every refund counts failed ones that refunded nothing.
