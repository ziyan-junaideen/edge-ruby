...
**`event.slug` is not the event code.** The API serves the code as two
separate attributes, and neither a fetched event nor a delivery ever carries
the joined form.
...
include one. A fetched one does.
Order by your own receipt time, not by an attribute that is nil half the time.

## Event codes

The sixteen Edge documents for webhook subscriptions. These are
...
\* `transaction.payment_demands.refunded` can still be subscribed to, and
nothing emits it. Edge retired the `refunded` payment demand state: a refund,
partial or full, leaves the demand `succeeded`. Watch the refund demand events
instead.

Note Edge's two lists disagree: the events documentation omits the
`consumer.payment_methods` rows that the webhook subscription documentation
includes. The subscription list is the one that governs what you can subscribe
to.
...
`data.attributes.state` for `succeeded`. A failed refund emits only
`transaction.refund_demands.failed`. `updated` also fires when a refund enters
`processing`, so an `updated` delivery on its own is not a success. There is no
longer an `errored` state; Edge removed it. See
[refund-demands.md](refund-demands.md#events).
...
The server treats `status: "archived"` specially and otherwise **drops**
`status`, `archived_at` and the merchant, and `secret_key` is only ever set on
create — so `update(id, status: "paused")` was
...
**Both list endpoints answer 500 today.** The routes exist and this client builds the right requests; what comes back is
