...
# The manifest is the contract every resource class is built against,
...
RSpec.describe "contract/manifest.yml" do
  ...
  it "keeps its caveats with it" do
    expect(manifest["caveats"]).to be_an(Array).and(be_any { |c| c.include?("sensitive_fields") })
  end
  ...
  describe "every resource" do
    ...
    it "points every relationship at a known resource" do
      types = resources.values.map { |spec| spec["json_api_type"] }
      # Targets are named in CamelCase; compare on a normalized form.
      known = types.to_set { |type| type.split("_").map(&:capitalize).join }
  ...
  describe "write safety" do
    it "marks only the resource whose replay contract has been exercised" do
      ...
      # payment_demands *documents* a replay contract — the API describes
      # idempotency_key as "a unique value that prevents double charging" — and
      # does not have one, so it is excluded on the evidence rather than on the
      # documentation. Exact, not a floor: adding a resource here must be a
      # deliberate act backed by a sandbox run.
  ...
  describe "attribute provenance" do
    ...
    # failure at the moment the manifest grows a new one.
    ...
    it "marks writable exactly the attributes the write path really refuses" do
      # `readonly: true` in the API's documentation is not enforced on writes,
      # so writability cannot be derived from it alone. On payment_demands the
      # flag is right about four attributes and wrong about seven — the seven
      # 3DS fields are accepted on create, and payer_timezone is required.
  ...
  describe "recorded server bugs" do
    ...
    # client's workaround becomes removable and the docs stale.
    it "still reports financial_institutions under the beneficial_owners type" do
      expect(resources.dig("financial_institutions", "json_api_type")).to eq("beneficial_owners")
    end

    it "still has no amount_refunded_cents outside the snapshot" do
    ...
    it "records the two confirm actions and no capture or void route" do
