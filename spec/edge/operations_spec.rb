...
RSpec.describe Edge::Operations do
  ...
  describe "only the operations the endpoint has" do
    it "gives a payment method no way to be created, updated or deleted" do
      # /v2/payment_methods supports list and retrieve only. A `.create` that
      # existed and returned 404 would read as a server fault rather than as a
      # capability the API does not have — and card data is meant to reach the
      # vault from the browser, never from a merchant's server.
  ...
  describe ".retrieve" do
    ...
    it "escapes a space as %20, not as +" do
      ...
      # body, and would answer 400 for a mismatch nobody could see.
  ...
  describe ".create" do
    ...
    it "sends the JSON:API type, not the route name, when they differ" do
      ...
      # "beneficial_owners". That endpoint is read-only, so the divergence is
      # reproduced here rather than borrowed — without it, every resource this
      # client writes to has the two the same and nothing would notice the wrong
      # one being used.
    ...
    it "resolves the related type through the target name, not the relationship name" do
      # `consumer_addresses.customer` points at the target `Customers`. A naive
      # `target.downcase` gives "customers" here by luck; the relationship named
    ...
    it "refuses to unset a relationship, which the API answers with a 500" do
      # The server has no handling for `{"data": null}` linkage.
    ...
    it "refuses a relationship the API sets itself" do
      ...
      # gets past validation and fails with a 500 rather than a 422.
      expect do
        Edge::ConsumerAddress.create({}, client: client,
    ...
    it "puts relationships inside data, where the server reads them" do
      # The server reads relationships from inside `data`. A sibling of `data`
    ...
    describe "retriable: true" do
      ...
      # replay lookup misses and creates a second refund, so both halves are
      # checked before a write may repeat.
      ...
      it "refuses a payment demand, whose replay contract does not exist" do
        ...
        # API documents idempotency_key as "a unique value that prevents
        ...
        # Supplying a key must not make this look safe.
        demand = Class.new(Edge::Resource) { contract "payment_demands" }
  ...
  describe "attribute names" do
    ...
    it "refuses a misspelled attribute in strict mode and suggests the right one" do
      # The server drops an attribute it does not declare, so a typo returns
      # 200 and changes nothing at all.
      strict = Edge::Client.new(api_key: secret, strict: true)
