...
RSpec.describe Edge::Relationship do
  ...
  describe "a belongs-to that carries linkage" do
    ...
    it "resolves the record from the same document, as the right class" do
      # Through the resource name the manifest records, not the reported type:
      # a type can belong to another resource entirely.
  ...
  describe "#fetch" do
    it "gets a to-one as the related record" do
      ...
      # resource linkage JSON:API specifies for this link.
  ...
  describe "#loaded?" do
    ...
    # an unlinked one are the same bytes, but the JSON:API meaning of
