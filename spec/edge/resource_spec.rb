...
RSpec.describe Edge::Resource do
  ...
  describe "contract-derived readers" do
    ...
    it "reports the JSON:API type the manifest records, which is not the route name" do
      # They diverge for financial_institutions, so the two are read from
  ...
  describe "identity" do
    ...
    it "is not shared across classes that report the same type" do
      # financial_institutions and beneficial_owners report one type. Equality
      # must not merge them.
  ...
  describe "every resource in the manifest" do
    ...
    it "shadows only the attributes already known to collide" do
      ...
      # `detail["type"]`. Anything else appearing here is a new collision and
    ...
    it "gives a reader only to attributes a server actually serves" do
      ...
      # the server never sends it. A reader is a promise the value is there;
