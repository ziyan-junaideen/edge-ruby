...
RSpec.describe Edge::Webhook do
  # A delivery body in the shape Edge sends for v3:
  # `{"data": {…event resource…}}`, with the five attributes a delivery carries
  # and no created_at.
  let(:body) do
    JSON.generate(
      "data" => {
        "id" => "evt_1", "type" => "events",
        # The shape Edge stores and delivers: the event code is split,
        # `resource_type` carrying the domain and route name and `slug`
        # carrying only the verb. Copied from Edge's own API documentation —
        # not invented, which is how the first version of this fixture hid a
        # real bug.
        "attributes" => {
  ...
  describe "the wire format" do
    ...
    # independent implementation over a payload with multi-byte UTF-8 in both
    ...
    it "matches the server's HMAC byte for byte" do
      independent_secret = "whsec_TestSecret_éß"
      independent_body = '{"data":{"id":"evt_1","type":"events","attributes":' \
                         '{"slug":"transaction.payment_demands.succeeded",' \
                         '"data":{"amount":"5.00 €"}}}}'

      signature = described_class.test_signature(independent_body, independent_secret,
                                                 timestamp: 1_756_742_400)
