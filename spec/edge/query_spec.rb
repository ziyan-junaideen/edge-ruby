...
RSpec.describe Edge::Query do
  ...
  describe "comparison operators" do
    ...
    it "refuses a comparison across a relationship" do
      # The server only strips the suffix from a single-segment key, so
    ...
    it "refuses a comparison against null, however it is spelled" do
      # The server trims and then reads both "null" and "NULL" as nil before
      # rejecting a comparison against it, so all four of these end up as the
      # same dropped filter.
  ...
  describe "values the server cannot represent" do
    it "refuses a value containing a comma" do
      # There is no escape: the server splits every string value on ",".
    ...
    it "refuses a value that is blank only by the server's definition" do
      # The server trims all Unicode whitespace; Ruby's String#strip
    ...
    it "refuses a blank filter name" do
      # `filter[]=x` is parsed as a list rather than a named filter, and the
      # server errors on it rather than ignoring it.
  ...
  describe "strict mode" do
    ...
    it "refuses filtering on a to-many relationship" do
      # The server filters across a belongs-to only and drops everything else,
      # so this would return every customer. The manifest records cardinality,
      # so it is catchable.
    ...
    it "refuses a comparison operator on a relationship" do
      # The suffix is stripped, the name resolves to a relationship, and the
      # server refuses anything but equality there.
