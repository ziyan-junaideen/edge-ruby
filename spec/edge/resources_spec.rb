...
RSpec.describe "the resource classes" do
  ...
  describe Edge::ConsumerAddress do
    ...
    it "reads the address lines with digits in their names" do
      # `line_1` and `line_2` were once missing from the manifest's field list
      # entirely, and they are marked sensitive.
  ...
  describe Edge::PaymentMethod do
    ...
    it "reads an expiry the server sent" do
      ...
      # fixture is what the payload becomes once the server serializes the
      # fields.
    ...
    it "reports no expiry for the payload the API actually sends" do
      # Verified against a running instance: the API documents expiry_month
      # and expiry_year and never sends either. A caller must
