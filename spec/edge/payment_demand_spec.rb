...
RSpec.describe Edge::PaymentDemand do
  ...
  describe "the two kinds of record this endpoint returns" do
    ...
    # intent and serializes it exactly like a payment demand. Both were
  ...
  describe "state predicates" do
    ...
    it "has no refunded? predicate, because a refunded demand stays succeeded" do
      # Edge retired the `refunded` state: partial and full refunds alike
      # leave the demand `succeeded`. A predicate that is never true would
      # read as "nothing was refunded".
      expect(demand({})).not_to respond_to(:refunded?, :amount_refunded_cents)
      expect(demand("processor_state" => "refunded")).not_to be_state_known
      expect(demand("processor_state" => "refunded")).not_to be_demand
    end
  end
  ...
  describe ".create" do
    ...
    it "sends the 3DS results the API requires, which its own docs call read-only" do
      # Six attributes are documented `readonly: true` and are accepted on
      # create anyway; `payer_timezone` is required. Recording them as
      # unwritable made this request impossible to build.
    ...
    it "still refuses the attributes the API really does compute" do
      # `fee_cents` is overwritten by the server on create, so sending it
      # cannot change anything; the AVS results are not accepted on create at
      # all. (`fee_cents` *is* writable on update — nothing recomputes it
      # there — and is refused anyway: letting a caller set their own fee is not
  ...
  describe ".update" do
    ...
    it "sends the attributes an update applies" do
    ...
    it "refuses the attributes update accepts and silently discards" do
      # An update applies the charge, addendum and settings groups and the
      # relationship ids — not the 3DS results, not capture_method, not
      # idempotency_key. The endpoint answers 200 and changes nothing, which is
      # the failure reject_readonly! exists for and which the resource-level
      # `writable` flag cannot express: it has no per-operation dimension.
  ...
  describe "#confirm" do
    ...
    it "sends the record's own identity and no attributes" do
      # The server reads only the include list off the payload, so any
