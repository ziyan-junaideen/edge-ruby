...
RSpec.describe "the refund, event and webhook resources" do
  ...
  describe Edge::RefundDemand do
    ...
    describe "states" do
      it "counts succeeded and failed as settled, and nothing else" do
        expect(refund("state" => "succeeded")).to be_settled
        expect(refund("state" => "failed")).to be_settled
        expect(refund("state" => "pending")).not_to be_settled
        expect(refund("state" => "processing")).not_to be_settled
      end

      it "has no errored? predicate, because the state was retired" do
        # Edge removed `errored`, which nothing ever transitioned into. A
        # predicate that can never be true reads as "this refund did not
        # error", which is not an answer.
        expect(refund({})).not_to respond_to(:errored?)
        expect(refund("state" => "errored")).not_to be_state_known
      end
    ...
    describe ".create" do
      # Every refund needs the demand it refunds. The server reads the linkage
      # before any validation runs, so omitting it is a 500 rather than a 422.
      ...
      it "sends amount_currency through when it is the one currency there is" do
        # It used to 500 on the documented "USD". It is now checked against the
        # payment demand's currency, so the matching value is harmless.
        stub_request(:post, "#{base}/v2/refund_demands")
          .to_return(status: 201, body: JSON.generate(created))

        create({ reason: "duplicate_charge", amount_cents: 100, amount_currency: "USD" })

        sent = a_request(:post, "#{base}/v2/refund_demands").with do |request|
          JSON.parse(request.body).dig("data", "attributes", "amount_currency") == "USD"
        end

        expect(sent).to have_been_made
      end

      it "refuses any other currency, since an unknown code is still a 500" do
        # With amount_cents present, the server errors on a currency code it
        # does not know before validating it.
        expect { create({ reason: "duplicate_charge", amount_cents: 100, amount_currency: "XYZ" }) }
          .to raise_error(ArgumentError, /must be "USD"/)
        expect { create({ reason: "duplicate_charge", amount_currency: "EUR" }) }
          .to raise_error(ArgumentError, /must be "USD"/)
        expect { create({ reason: "duplicate_charge", amount_currency: nil }) }
          .to raise_error(ArgumentError, /must be "USD"/)
      end

      it "refuses attributes the API does not apply" do
        # `state` above all: every new refund starts pending whatever was sent,
        # so a caller setting it gets a 201 and no effect.
      ...
      it "sends a partial amount as given, leaving the cap to the server" do
        # Several partial refunds are allowed against one demand, up to its
        # total. The server checks the remaining balance under a row lock; a
        # client-side check would race, so none is made here.
        amounts = []
        stub_request(:post, "#{base}/v2/refund_demands").to_return do |request|
          amounts << JSON.parse(request.body).dig("data", "attributes", "amount_cents")
          { status: 201, body: JSON.generate(created) }
        end

        create({ reason: "customer_canceled", amount_cents: 25_00 })
        create({ reason: "customer_canceled", amount_cents: 10_00 })

        expect(amounts).to eq([25_00, 10_00])
      end

      it "surfaces an over-refund as the server's validation error" do
        stub_request(:post, "#{base}/v2/refund_demands").to_return(
          status: 422,
          headers: { "Content-Type" => "application/vnd.api+json" },
          body: JSON.generate(
            "errors" => [{
              "status" => "422",
              "title" => "refund amount ($100.01) is greater than the unrefunded amount " \
                         "on this payment ($100.00)",
              "source" => { "pointer" => "/data/attributes/amount_cents" }
            }]
          )
        )

        expect { create({ reason: "duplicate_charge", amount_cents: 100_01 }) }
          .to raise_error(Edge::InvalidRequestError, /unrefunded amount/)
      end

      it "sends no amount when none is given, so the server refunds what remains" do
        stub_request(:post, "#{base}/v2/refund_demands")
          .to_return(status: 201, body: JSON.generate(created))
    ...
    it "offers no refunded-balance arithmetic, because the server serializes none" do
      # The server tracks the balance internally to cap refunds, but no
      # response carries it. Arithmetic here would be over a list the caller
      # fetched at some earlier moment, racing every in-flight refund.
  ...
  describe Edge::Event do
    ...
    it "joins the event code the server keeps in two attributes" do
      # The server keeps resource_type "transaction.payment_demands" and slug
      # "succeeded"; the joined form is used only internally to match
      # subscriptions, and is never stored or delivered.
    ...
    it "reports no created_at for a delivered event, which does not carry one" do
      # A delivery carries mode, resource_type, resource_id, slug and data. A
      # fetched event has created_at; a delivered one does not, and a consumer
      # ordering by it would be ordering by nil.
  ...
  describe Edge::WebhookSubscription do
    ...
    describe ".update" do
      it "refuses a status change, which the API answers 200 and discards" do
        # The server special-cases status "archived" and otherwise drops
        # status, archived_at and the merchant from an update. A
      ...
      it "accepts the attributes an update really applies" do
