...
module Edge
  ...
  # A filter it fails to resolve is **dropped**, not rejected, and production
  # does not paginate — so one mistyped filter turns a narrow lookup into a
  # full-table fetch that still returns 200. Every rule enforced here marks a
  # case where the request the caller wrote is not the request the server would
  # run.
  #
  ...
  #   #     "sort"                     => "-created_at"}
  class Query
    # Ordered as the server orders them, longest first, so `amount_cents_gte`
    # resolves to `gte` rather than to `gt`.
    COMPARISON_SUFFIXES = { "_gte" => :gte, "_lte" => :lte, "_gt" => :gt, "_lt" => :lt }.freeze

    OPERATORS = { eq: "", gt: "_gt", gte: "_gte", lt: "_lt", lte: "_lte" }.freeze

    # `"null"` and `"NULL"` are read by the server as `IS NULL`. It follows
    # that the literal string "null" cannot be filtered for at all; `nil` is
    ...
    # Descending sort is a leading `-` and a dotted path is a relationship
    # chain; both are passed through as written.
    ...
    def fields_params
      ...
      pairs(@fields).each_with_object({}) do |(type, names), result|
        ...
        # financial_institutions, where the type belongs to another
