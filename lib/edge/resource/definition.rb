...
module Edge
  class Resource
    ...
    module Definition
      ...
      # type. They can differ: `financial_institutions` reports another
      # resource's type.
      #
      ...
      # honouring it was how the field nearly shipped.
      #
      ...
      # `type`, which JSON:API 1.1 §5.2 forbids precisely because it collides
      # with the resource object's own `type`. So the attribute is read as
      # `detail["type"]`.
      #
      ...
      # belong to another resource entirely, so a type registry would
      ...
      # one of the three relationship shapes it has, so a reader that
      ...
      # `from` records where the attribute is known from: `view` means the
      # server serializes it, and anything else means it is only documented.
