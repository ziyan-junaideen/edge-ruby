...
module Edge
  ...
  #
  # Each is a separate module, and `contract` extends only the ones the manifest
  # says the endpoint has. That is the point: `PaymentMethod.create` raises
  # NoMethodError, because `/v2/payment_methods` is exposed for listing and
  # retrieval only, and there is no create route anywhere in the API. A method
  # that existed and returned a 404 would be worse — it would look like a server
  # problem rather than a capability the API does not have.
  module Operations
    ...
    module Support
      ...
      # id `"a+b"` in the URL and `"a b"` in the body, and would reject the
      # mismatch with a 400 nobody could explain.
      ...
      # lookup requires an idempotency key, and a request without one misses
      # and creates a second record. On a refund that is a second
    ...
    module Body
      ...
      # the route name — they can differ (`financial_institutions` reports
      # another resource's type) and the server matches on the type.
      def body_for(attributes, relationships, id: nil)
        data = { "type" => json_api_type, "attributes" => stringify(attributes) }
        data["id"] = id.to_s if id
        # Inside `data`, which is where the server reads them from and where
        # JSON:API puts them.
      ...
      # An attribute the API does not declare is dropped without comment, so a
      # misspelled name returns 200 and leaves the record unchanged.
      #
      # Names are only checked in strict mode, for the same reason Query's are:
      # the manifest is maintained by hand, and one that had fallen behind the
      # server would refuse a field that really is writable. What is checked
      # always is the 30 attributes the manifest records as `writable: false`,
      # which is positive knowledge rather than an absence of it.
      ...
      # The server has no handling for `{"data": null}` linkage, so the request
      # is a 500 rather than an unset.
      def reject_unsettable!(name)
        raise ArgumentError,
              "#{contract_name}.#{name} cannot be unset: the API has no handling for null " \
              "linkage and answers with a 500."
      end

      # The relationship must exist and be writable. The server rejects a name
      # it does not recognise; for one it recognises but cannot write, the
      # request is a 500.
      ...
      # Resolved through the resource name the manifest records rather than the
      # reported type, which can belong to another resource entirely
    ...
    module Update
      ...
      # `idempotent_writes` is recorded per resource, but the only replay the
      # API performs is on the refund *create* path. Nothing replays a PATCH,
      # so a resource-level flag cannot speak for this operation.
      ...
      def reject_retriable_update!
        raise ArgumentError,
              ...
              "create path. Nothing replays a PATCH."
      end
    end
