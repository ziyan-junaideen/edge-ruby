...
module Edge
  class Query
    ...
    class Filters
      ...
      # Read by the server as `IS NULL`, after the value has already been
      # trimmed. Both spellings, and only these two.
      ...
      # `filter[]=x` is not an empty-named filter, it is a list, and the server
      # expects a map of named filters — so the request errors rather than
      # being ignored. `filter: { params[:field] => query }`
      ...
      # A comma is the value separator and there is no escape for it: the
      # server splits every string value on ",". A value that
      ...
      # A blank value is skipped. Skip them all and the value list is empty,
      # which drops the filter — and the request succeeds, returning every
      # record, because production does not paginate. An empty search box must
      # not fetch the whole table.
      ...
      def validate_comparison!(path, encoded, suffix, operator)
        ...
        # padded " null " and an upper-case "NULL" both become nil, and a
        # comparison against nil is then dropped.
      ...
      # The operator is only stripped from a single-segment key. On a dotted
      # path the whole last segment is read as a field name, no such field
      # exists, and the filter is dropped.
