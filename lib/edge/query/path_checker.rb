...
module Edge
  class Query
    ...
    class PathChecker
      ...
      # operator but equality on a relationship, and drops the filter rather
      # than saying so.
      ...
      # manifest records its own gaps, and refusing a request over a field it
      # merely does not know about would be worse than sending it.
      ...
      # `filter[payer]` and `filter[payer.id]` are the same request: both match
      ...
      # That only works across a belongs-to; for every other association the
      # filter is dropped. The manifest cannot tell a belongs-to from a has-one,
      ...
      def related_resource!(spec, field, label, resource_name)
        ...
        # Resolved through the resource name the manifest records rather than
        # the JSON:API type: an endpoint can report a type belonging to another
        # resource entirely (`financial_institutions`), and the name does not
        # lie.
