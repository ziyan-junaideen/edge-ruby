...
module Edge
  class Query
    ...
    module Values
      ...
      # What the server does to every filter value before it is cast: trim all
      # Unicode whitespace. Ruby's own `String#strip` is not the same function:
      # it removes ASCII whitespace and NUL only, so a non-breaking space —
      # which a browser paste produces routinely — looks like content here and
      # is trimmed away to nothing there.
