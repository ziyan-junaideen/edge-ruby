...
module Edge
  ...
  Request = Struct.new(:verb, :url, :body, :params, :headers, :retriable, keyword_init: true) do
    ...
    # financial_institutions, which reports another resource's type.
    #
    # A collection or a member, and nothing deeper. A sub-resource path is a
    # different operation from the resource it hangs off, and answering with the
    # parent's name would hand it the parent's contract:
    # `PATCH /v2/payment_demands/<id>/confirm` would inherit `payment_demands`'
    # `idempotent_writes` and could be marked retriable, when confirm is a
    # *retry of a failed charge* and replaying it charges again. `nil` denies
    # the lookup, and Transport's `reject_unsafe_replay!` then refuses
    # `retriable: true` — which is the right answer for every sub-resource this
    # API has. Inline rather than a named constant: a constant assigned inside a
    # `Struct.new` block resolves against the block's lexical scope, so it would
    # land on `Edge` rather than on Request.
    def resource_name
