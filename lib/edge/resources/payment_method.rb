...
module Edge
  ...
  #
  # **Read-only, and not by choice of this client.** `/v2/payment_methods`
  # supports listing and retrieval only; there is no create, update or delete
  # route anywhere in the API. `Edge::PaymentMethod` therefore has no `.create`
  # at all, rather than one that would return a 404 and read as a server fault.
  #
  ...
  class PaymentMethod < Resource
    ...
    #
    # **Returns `[nil, nil]` against today's API.** The API documents both
    # fields but never serializes them, so no payment method has ever carried
    # either — verified against a running instance.
    #
    # Kept rather than removed because the fix is Edge's to make, after which
    # this reader works unchanged. `#expiry_known?` is how a caller
