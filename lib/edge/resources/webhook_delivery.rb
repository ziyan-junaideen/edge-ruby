...
module Edge
  ...
  #
  # **`GET /v2/webhook_deliveries` answers 500 today.**
  class WebhookDelivery < Resource
    contract "webhook_deliveries"

    # True when at least one attempt has failed.
    #
    # Deliberately not called `retried?`. `fails_count` is incremented on every
    # failure, but a 400, 401, 403, 404 or 405 is not retried at all — so a
    # delivery your endpoint 404'd has a count of one and was never retried. It
    # also does not mean "undelivered": one that failed twice and then succeeded
    # still reports two.
