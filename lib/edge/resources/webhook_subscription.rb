...
module Edge
  ...
  #
  # **`GET /v2/webhook_subscriptions` answers 500 today.** The route exists and
  # this client builds the right request; what comes back is a server fault.
  class WebhookSubscription < Resource
    contract "webhook_subscriptions"

    # What an update actually applies. The server also drops `merchant_id` and
    # `mode` from an update.
    ...
    class << self
      ...
      # `status`, `archived_at`, `merchant` and `merchant_id` outright, while
      # `secret_key` is only ever set on create.
      #
      ...
      def reject_undroppable!(written)
        ...
        raise ArgumentError,
              ...
              "API drops status, archived_at and the merchant from an update, " \
